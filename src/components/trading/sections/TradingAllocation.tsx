...
const DestinationCashAccountsSection: React.FC<{
  ...
  const handleAllocationChange = (accountId: string, amount: number) => {
    ...
    const cashAllocations = Object.entries(updatedAllocations)
      ...
      .map(([destinationId, amount]) => {
        const account = mockCashAccountsFlat.find(acc => acc.id === destinationId);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gray-50 p-4 rounded-md border">
        <div className="flex justify-between mb-2">
          <span>Total amount to allocate:</span>
          <span className="font-medium">
            {totalAmount.toLocaleString('en-US', {
              style: 'currency',
              currency: currency
...
// Reusable component for allocation row in institutions view
